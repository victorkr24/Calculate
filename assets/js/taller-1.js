...
//Codigo de la suma 
function summaNumeros(dato1 , dato2){
    return dato1 + dato2;
}
...
function checkInputsSumma(l1, l2) {
    if (Number.isNaN(l1) || l1 <= 0) {
        e = document.getElementById('errorSumma');
        e.innerHTML = 'Ingrese un valor válido para numero1';
        return false;
    }
    if (Number.isNaN(l2) || l2 <= 0) {
        e = document.getElementById('errorSumma2');
        e.innerHTML = 'Ingrese un valor válido para numero2';
        return false;
    }

    return true;
}
function renderResultSumma(re,cale){
    const overlay = document.getElementById('overlay');
    overlay.style.display = 'grid';

    const unit = cale == 'Summa' ? 'total<span>.<span>':'';
    const resultIMG = document.getElementById('resultIMG');
    const resultH2 = document.getElementById('resultH2');        
    const resultH4 = document.getElementById('resultH4');
    resultIMG.src = 'assets/img/square.png';
    resultH2.innerHTML = 'Summa';

    resultH4.innerHTML = `La <strong>${cale}</strong>  <strong></strong> es: <strong class="unit"> 
    ${re} ${unit}</strong>`;
}

function calcularSumma(){
    const dato1 = parseInt(document.getElementById('inputSumma').value);
    const dato2 = parseInt(document.getElementById('inputSumma2').value);

    if(checkInputsSumma(dato1,dato2)){
        const result = summaNumeros(dato1, dato2);
        renderResultSumma(result,'Summa')
    }
